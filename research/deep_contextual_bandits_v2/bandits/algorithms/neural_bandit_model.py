...
class NeuralBanditModel(BayesianNN):
  ...
  def build_layer(self, x, num_units):
    ...
    nn = tf.contrib.layers.fully_connected(
        ...
        normalizer_params={},
        weights_initializer=tf.compat.v1.random_uniform_initializer(-init_s, init_s)
    )

    if dropout:
      nn = tf.nn.dropout(nn, 1 - (self.hparams.keep_prob))
  ...
  def forward_pass(self):
    ...
    with tf.compat.v1.variable_scope(scope_name, reuse=tf.compat.v1.AUTO_REUSE):
      ...
      y_pred = tf.compat.v1.layers.dense(
          ...
          kernel_initializer=tf.compat.v1.random_uniform_initializer(-init_s, init_s))
  ...
  def build_model(self):
    ...
    with self.graph.as_default():
      ...
      with tf.compat.v1.name_scope(self.name):
        ...
        self.loss = tf.math.squared_difference(self.y_pred, self.y)
        self.weighted_loss = tf.multiply(self.weights, self.loss)
        self.cost = tf.reduce_sum(input_tensor=self.weighted_loss) / self.hparams.batch_size
        ...
        grads, _ = tf.clip_by_global_norm(
            tf.gradients(ys=self.cost, xs=tvars), self.hparams.max_grad_norm)
  ...
  def create_summaries(self):
    ...
    with self.graph.as_default():
      with tf.compat.v1.name_scope(self.name + "_summaries"):
