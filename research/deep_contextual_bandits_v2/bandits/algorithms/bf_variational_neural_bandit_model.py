...
class BfVariationalNeuralBanditModel(BayesianNN):
  ...
  def build_layer(self, input_x, input_x_local, shape,
    ...
    output_h_local = m_h + tf.sqrt(v_h + 1e-6) * tf.random.normal(tf.shape(input=v_h))
  ...
  def build_model(self, activation_fn=tf.nn.relu):
    ...
    def weight_prior(dtype, shape, c, d, e):
      ...
      return tfd.Independent(
          tfd.Normal(loc=tf.zeros(shape, dtype),
                     scale=tf.cast(np.sqrt(2) / shape[0], dtype=tf.float32)),
          reinterpreted_batch_ndims=tf.size(input=shape))
    ...
    batch_size = tf.cast(tf.shape(input=self.x)[0], dtype=tf.float32)
    weighted_log_likelihood = tf.reduce_sum(
        input_tensor=log_likelihood * self.weights) / batch_size
  ...
  def build_graph(self):
    ...
    Placeholders are: n (size of the datasets), x and y (context and observed
