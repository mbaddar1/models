...
class MultitaskGP(BayesianNN):
  ...
  def sq_dist(self, x, x2):
    a2 = tf.reduce_sum(input_tensor=tf.square(x), axis=1)
    b2 = tf.reduce_sum(input_tensor=tf.square(x2), axis=1)
    sqdists = tf.expand_dims(a2, 1) + b2 - 2.0 * tf.matmul(x, tf.transpose(a=x2))
    return sqdists
  ...
  def task_cov(self, x, x2):
    ...
    x_vecs = tf.gather(self.task_vectors, tf.argmax(input=x, axis=1), axis=0)
    x2_vecs = tf.gather(self.task_vectors, tf.argmax(input=x2, axis=1), axis=0)
  ...
  def build_model(self):
    ...
    index_counts = self.atleast_2d(tf.reduce_sum(input_tensor=self.weights, axis=0),
                                   self.n_out)
    index_counts = tf.compat.v1.where(index_counts > 0, index_counts,
                            tf.ones(tf.shape(input=index_counts), dtype=tf.float64))
    self.mean_op = tf.compat.v1.assign(self.input_mean,
                             tf.reduce_sum(input_tensor=self.y, axis=0) / index_counts)
    self.var_op = tf.compat.v1.assign(
        self.input_std, tf.sqrt(1e-4 + tf.reduce_sum(input_tensor=tf.square(
            self.y - tf.reduce_sum(input_tensor=self.y, axis=0) / index_counts), axis=0)
                                / index_counts))

    with tf.control_dependencies([self.var_op]):
      y_normed = self.atleast_2d(
          (self.y - self.input_mean) / self.input_std, self.n_out)
      y_normed = self.atleast_2d(tf.boolean_mask(tensor=y_normed, mask=self.weights > 0), 1)
    ...
    with tf.control_dependencies([self.input_op, self.input_w_op, self.out_op]):
      self.self_cov = (self.cov(self.x_in, self.x_in) *
                       ...
                       tf.eye(tf.shape(input=self.x_in)[0], dtype=tf.float64) * alpha)

    self.chol = tf.linalg.cholesky(self.self_cov)
    self.kinv = tf.linalg.cholesky_solve(self.chol, tf.eye(tf.shape(input=self.x_in)[0],
                                                    dtype=tf.float64))
    ...
    with tf.control_dependencies([self.input_cov_op]):
      logdet = 2.0 * tf.reduce_sum(input_tensor=tf.math.log(tf.linalg.tensor_diag_part(self.chol) + 1e-16))

    # Log Marginal likelihood
    self.marginal_ll = -tf.reduce_sum(input_tensor=-0.5 * tf.matmul(
        tf.transpose(a=y_normed), tf.matmul(self.kinv, y_normed)) - 0.5 * logdet -
  ...
  def posterior_mean_and_sample(self, candidates):
    ...
    inds = tf.squeeze(
        tf.reshape(
            tf.tile(
                ...
                (1, tf.shape(input=candidates)[0])), (-1, 1)))
    ...
    test_sqrt = tf.matmul(v, tf.matmul(tf.linalg.tensor_diag(s), tf.transpose(a=v)))

    y_sample = (
        tf.matmul(
            test_sqrt,
            tf.random.normal([tf.shape(input=test_sqrt)[0], 1], dtype=tf.float64)) +
        y_mean)

    y_sample = (
        tf.transpose(a=tf.reshape(y_sample,
