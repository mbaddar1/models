...
FLAGS = flags.FLAGS
tfd =   tfp.distributions# update to: tensorflow_probability.distributions


def log_gaussian(x, mu, sigma, reduce_sum=True):
  res = tfd.Normal(mu, sigma).log_prob(x)
  if reduce_sum:
    return tf.reduce_sum(input_tensor=res)
  else:
    return res
...
class BBAlphaDivergence(BayesianNN):
  ...
  def sample_neural_network(self, activation_fn=tf.nn.relu):
    ...
    with self.graph.as_default():
      ...
      for layer_id in range(self.total_layers):
        ...
        log_f += tf.reduce_sum(input_tensor=t1 + t2)
      ...
      weighted_log_likelihood = tf.reduce_sum(input_tensor=log_likelihood * self.weights, axis=-1)
  ...
  def log_z_q(self):
    ...
    with self.graph.as_default():
      ...
      for layer_id in range(self.total_layers):
        ...
        w_term = 0.5 * tf.reduce_sum(input_tensor=w_mu ** 2 / w_sigma ** 2)
        w_term += 0.5 * tf.reduce_sum(input_tensor=tf.math.log(2 * np.pi) + 2 * tf.math.log(w_sigma))

        b_term = 0.5 * tf.reduce_sum(input_tensor=b_mu ** 2 / b_sigma ** 2)
        b_term += 0.5 * tf.reduce_sum(input_tensor=tf.math.log(2 * np.pi) + 2 * tf.math.log(b_sigma))
  ...
  def log_alpha_likelihood_ratio(self, activation_fn=tf.nn.relu):
    ...
    nn_log_ratio = self.alpha * tf.transpose(a=nn_log_ratio)
    logsumexp_value = tf.reduce_logsumexp(input_tensor=nn_log_ratio, axis=-1)
    log_k_scalar = tf.math.log(tf.cast(self.num_mc_nn_samples, tf.float32))
    log_k = log_k_scalar * tf.ones([self.batch_size])

    return tf.reduce_sum(input_tensor=logsumexp_value - log_k, axis=-1)
  ...
  def build_model(self, activation_fn=tf.nn.relu):
    ...
    self.cost = tf.reduce_sum(input_tensor=weighted_sq_loss) / self.batch_size
  ...
  def assign_lr(self):
    ...
    In bandits settings, we do expand our datasets over time. Then, we need to
