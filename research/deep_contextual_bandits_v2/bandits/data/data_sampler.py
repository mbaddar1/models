...
def sample_mushroom_data(file_name,
  ...
  Args:
    file_name: Route of file containing the original Mushroom UCI datasets.
  ...
  Returns:
    datasets: Sampled matrix with n rows: (context, eat_reward, no_eat_reward).
...
def sample_stock_data(file_name, context_dim, num_actions, num_contexts,
  ...
  """Samples linear bandit game from stock prices datasets.

  Args:
    file_name: Route of file containing the stock prices datasets.
    ...
    shuffle_rows: If True, rows from original datasets are shuffled.

  Returns:
    datasets: Sampled matrix with rows: (context, reward_1, ..., reward_k).
    opt_vals: Vector of expected optimal (reward, action) for each context.
  """
...
def sample_jester_data(file_name, context_dim, num_actions, num_contexts,
  ...
  """Samples bandit game from (user, joke) dense subset of Jester datasets.

  Args:
    file_name: Route of file containing the modified Jester datasets.
  ...
  Returns:
    datasets: Sampled matrix with rows: (context, rating_1, ..., rating_k).
...
def sample_statlog_data(file_name, num_contexts, shuffle_rows=True,
  ...
  """Returns bandit problem datasets based on the UCI statlog data.

  Args:
    file_name: Route of file containing the Statlog datasets.
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original datasets are shuffled.
    remove_underrepresented: If True, removes arms with very few rewards.

  Returns:
    datasets: Sampled matrix with rows: (context, action rewards).
    opt_vals: Vector of deterministic optimal (reward, action) for each context.
...
def sample_adult_data(file_name, num_contexts, shuffle_rows=True,
                      remove_underrepresented=False):
  """Returns bandit problem datasets based on the UCI adult data.

  Args:
    file_name: Route of file containing the Adult datasets.
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original datasets are shuffled.
    remove_underrepresented: If True, removes arms with very few rewards.
...
def sample_census_data(file_name, num_contexts, shuffle_rows=True,
                       remove_underrepresented=False):
  """Returns bandit problem datasets based on the UCI census data.

  Args:
    file_name: Route of file containing the Census datasets.
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original datasets are shuffled.
    remove_underrepresented: If True, removes arms with very few rewards.
  ...
  Note: this is the processed (not the 'raw') datasets. It contains a subset
  of the raw features and they've all been discretized.

  https://archive.ics.uci.edu/ml/datasets/US+Census+Data+%281990%29
  """
  # Note: this datasets is quite large. It will be slow to load and preprocess.
  with tf.io.gfile.GFile(file_name, 'r') as f:
...
def sample_covertype_data(file_name, num_contexts, shuffle_rows=True,
                          remove_underrepresented=False):
  """Returns bandit problem datasets based on the UCI Cover_Type data.

  Args:
    file_name: Route of file containing the Covertype datasets.
    num_contexts: Number of contexts to sample.
    shuffle_rows: If True, rows from original datasets are shuffled.
    remove_underrepresented: If True, removes arms with very few rewards.
